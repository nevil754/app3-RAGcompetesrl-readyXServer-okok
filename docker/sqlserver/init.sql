...
BEGIN  --inizio del corpo
    ...
    --crea tab conversation_summaries (long-term memory: summary conversazioni passate)
    SET @sql = '
    IF NOT EXISTS (
        SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
    ...
    --crea tab user_facts (long-term memory: fatti estratti via LLM sull utente)
    SET @sql = '
    IF NOT EXISTS (
        SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id
    ...
    --crea index su user_facts per lookup veloce per utente
    SET @sql = '
    ...
    --crea utente SQL Server senza login per EXECUTE AS USER (schema switching session-scoped)
    --richiesto da sqlserver.py: EXECUTE AS USER = N''usr_tenant_acme_corp'' imposta DEFAULT_SCHEMA
    SET @sql = '
    ...
    --concedi permessi DML sul schema tenant al nuovo utente
