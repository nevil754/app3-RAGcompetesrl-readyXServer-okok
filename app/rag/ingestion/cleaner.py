...
def clean_text(text: str) -> str:
    ...
    if repeated:
        lines = [ l for  l in lines if l.strip() not in repeated ]  #check che la line target non esista gia come chiave in repeated, se non esiste allora la aggiunge alla nuova lista lines
...
def normalize_whitespace(text: str) -> str:
    ...
    text = re.sub(r"[ \t]+", " ", text)     #uno o più spazi/tab  -> 1 space
    text = re.sub(r"\n{3,}", "\n\n", text)  #collassa 3/3+ newline consecutive  ->  2 linee 
    return text.strip()
