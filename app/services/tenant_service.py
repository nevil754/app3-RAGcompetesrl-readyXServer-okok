...
async def provision_tenant(    #🔥🔥🔥 provision_tenant() crea schema tenant + ensure_collection() crea collection qdrant (x my tenant) + crei utente admini iniziale
    ...
    await loop.run_in_executor( None, ensure_collection, slug )  #esegue funct sync da code async, in questo caso per CREARE COLLECTION QDRANT per il tenant 
    ...
    if admin_email and admin_password:
        ...
        async with tenant_db.aget_session(slug) as session:
            await session.execute(    #CREA NUOVO UTENTE ADMIN NELLO SCHEMA TENANT
                text("""
